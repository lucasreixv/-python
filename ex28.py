...
class Produto: #Classe dos produtos
    def __init__(self , nome ,preço): #init é o metodo construtor quando criamos automaticamente um produtototal
        self.nome = nome #self faz a ligação entre o objeto criado e os atributos/métodos da classe, permitindo que cada objeto tenha seus próprios valores.
        self.preço = preço
        
    def mostrar(self): #método mostrar() serve para mostrar os atributos do objeto criado pela classe Produto.
...
def listar_produtos(): #função para percorrer a lista de produtos
    if len(produtos) <= 0: #se a quantidade dentro da lista for 0,
        ...
        for i in range(len(produtos)): #percorre o indice em um raio de quantidade da lista produtos
            ...
            print(f"Produto {i + 1}- ,end="" ") #o end="" substituiu a quebra de linha por “nada”. o final vai ser nd
            #end="" é um parâmetro da função print() usado para definir o que será 
            #colocado no final da impressão,aq ele serve pra tirar quebra de linha automatica \n
            produto.mostrar()
